/*
pytorch resnet18分类模型，经过trtexec转换为trt模型。
trt api推理示例
    前处理 + 后处理
...
void inference(){
    ...
    TRTLogger logger;
    auto engine_data = load_file("./05_classify_demo/workspace/classifier.trt");
    ...
    ///////////////////////////////////////////////////
    // image to float
    auto image = cv::imread("./05_classify_demo/workspace/dog.jpg");
    ...
    auto labels = load_labels("./05_classify_demo/workspace/labels.imagenet.txt");
    auto predict_name = labels[predict_label];
    float confidence  = prob[predict_label];    // 获得预测值的置信度
    // 使用tensorrt7.2.3支持pytorch1.6.0,onnx1.6.0 若版本不一致会轻微影响精度，与pytorch版本有轻微差异
    // pytorch 0.32459262013435364 ,c++ trt: 0.324794
