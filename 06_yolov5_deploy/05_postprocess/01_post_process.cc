...
vector<Box> cpu_decode(float* predict, int rows, int cols, float confidence_threshold = 0.25f, float nms_threshold = 0.45f){
    ...
    std::vector<bool> remove_flags(boxes.size());  // 直接删除元素的移动过程费事，使用flags避免
    std::vector<Box> box_result;
    box_result.reserve(boxes.size());  // 提前分配capcity
...
int main(){
    ...
    auto data = load_file("./06_yolov5_deploy/05_postprocess/workspace/predict.data");
    auto image = cv::imread("./06_yolov5_deploy/05_postprocess/workspace/input-image.jpg");
    ...
    cv::imwrite("./06_yolov5_deploy/05_postprocess/workspace/image-draw.jpg", image);
    return 0;
}