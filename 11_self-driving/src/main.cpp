...
static void inference(){
    ...
    auto yolov5 = Yolo::create_infer("11_self-driving/workspace/yolov5s.trt", Yolo::Type::V5, 0, 0.25, 0.45);
    auto road = Road::create_infer("11_self-driving/workspace/road-segmentation-adas.trt", 0);
    auto ldrn = Ldrn::create_infer("11_self-driving/workspace/ldrn_kitti_resnext101_pretrained_data_grad_256x512.trt", 0);
    auto lane = Lane::create_infer("11_self-driving/workspace/new-lane.trt", 0);

    cv::Mat image, scence;
    cv::VideoCapture cap("11_self-driving/workspace/4k-tokyo-drive-thru-ikebukuro.mp4");
    float fps = cap.get(cv::CAP_PROP_FPS);
    int width  = cap.get(cv::CAP_PROP_FRAME_WIDTH);
    int height = cap.get(cv::CAP_PROP_FRAME_HEIGHT);
    scence = cv::Mat(height * 1.5, width * 2, CV_8UC3, cv::Scalar::all(0));
    cv::VideoWriter writer("11_self-driving/workspace/output.mp4", cv::VideoWriter::fourcc('M', 'P', 'G', '2'), fps, scence.size());
