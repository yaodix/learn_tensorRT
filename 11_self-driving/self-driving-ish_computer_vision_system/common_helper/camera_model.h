...
class CameraModel {
    /***
    * s[x, y, 1] = K * [R t] * [Mw, 1]
    *     K: カメラの内部パラメータ--照相机的内部参数
    *     [R t]: カメラの外部パラメータ--照相机的外部参数
    *     R: ワールド座標上でのカメラの回転行列 (カメラの姿勢)--照相机在世界坐标上的旋转矩阵(照相机的姿势)
    *     t: カメラ座標上での、カメラ位置(Oc)からワールド座標原点(Ow)へのベクトル= Ow - Oc--在相机坐标上，从相机位置(Oc)到世界坐标原点(Ow)的向量= Ow - Oc
    *         = -RT (T = ワールド座標上でのOwからOcへのベクトル) -- (T =世界坐标上从Ow到Oc的向量)
    *     Mw: ワールド座標上での対象物体の座標 (Xw, Yw, Zw)--对象物体在世界坐标上的坐标(Xw, Yw, Zw)
    * s[x, y, 1] = K * Mc
    *     Mc: カメラ座標上での対象物体の座標 (Xc, Yc, Zc)--照相机坐标上的对象物体的坐标(Xc, Yc, Zc)
    *         = [R t] * [Mw, 1]
    *
    * 理由: Mc = R(Mw - T) = RMw - RT = RMw + t   (t = -RT)
    *
    * 注意1: tはカメラ座標上でのベクトルである。そのため、Rを変更した場合はtを再計算する必要がある-
    *       t是照相机坐标上的向量。因此，如果改变R，就需要重新计算t。
    *
    * 注意2: 座標系は右手系。X+ = 右、Y+ = 下、Z+ = 奥  (例. カメラから見て物体が上にある場合、Ycは負値)
    *        -- 坐标系为右手系。X+ =右，Y+ =下，Z+ =里(例。从照相机看物体在上时，Yc为负值)
    ***/
